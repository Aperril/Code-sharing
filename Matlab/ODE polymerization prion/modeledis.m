function [Xl] = modeledis(t,X,Xlag,param) 
%Les parametres
...
%La dynamique
Xl=zeros(2*xf+x0+xs+2,1); %[c1,ci,c_{x0-1},c0,ua,cfj,cf0,cai,c0a,pc,psc,Psi]

%X_1=c1
Xl(1)=-4*1*a(1)*X(1)^2-X(1)*P+M-2*ae*X(1)^2-Pe*X(1)+Me;
...
%X_{x0}=c0
Xl(x0)=-gam*X(x0)-del*X(x0)*X(x0+2*xf)+(x0-1)*a(x0-1)*X(1)*X(x0-1)+del*Xlag(x0+2*xf)*Xlag(x0)-(eps/2)*X(x0);

%X_{x0+1}=ua
Xl(x0+1)=gam*X(x0); 

%X_{x0+2}=cf2
Xl(x0+2)=1*a(1)*X(1)^2+bf*X(x0+3)-X(x0+2)*(bf+2*a(2)*X(1))-gamf*X(x0+2);
...
%X_{x0+2xf+1}=psc
Xl(x0+2*xf+1)=alp*X(x0+2*xf)+del*Xlag(x0+2*xf)*Xlag(x0); 

%X_{x0+2xf+2}=Psi
Xl(x0+2*xf+2)=del*X(x0+2*xf)*X(x0)-del*Xlag(x0+2*xf)*Xlag(x0); 
